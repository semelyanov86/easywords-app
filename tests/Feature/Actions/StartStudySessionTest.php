...
final class StartStudySessionTest extends TestCase
{
    ...
    public function test_starts_new_study_session(): void
    {
        ...
        $this->assertIsArray(Cache::get("words.start.{$user->id}"));
        $this->assertCount(3, Cache::get("words.start.{$user->id}"));
        $this->assertIsInt(Cache::get("words.current.{$user->id}"));
        $this->assertIsInt(Cache::get("words.next.{$user->id}"));
        $this->assertNull(Cache::get("words.prev.{$user->id}"));
    }
    ...
    public function test_returns_existing_session_if_already_started(): void
    {
        ...
        Cache::put("words.start.{$user->id}", $wordIds);
        Cache::put("words.current.{$user->id}", $wordIds[0]);
    ...
    public function test_throws_exception_when_no_words_available(): void
    {
        ...
        $action->handle(userId: $user->id, limit: 5, reverse: false);
    }
